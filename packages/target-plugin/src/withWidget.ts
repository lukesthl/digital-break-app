...
const withWidget: ConfigPlugin<Props> = (config, props) => {
  ...
  if (props.icon && !/https?:\/\//.test(props.icon)) {
    props.icon = path.join(props.directory as any, props.icon);
  }

  const widgetDir = (props.name ?? path.basename(props.directory as any))
    .replace(/\/+$/, "")
    .replace(/^\/+/, "");
  ...
  const bundleId = (config.ios as any).bundleIdentifier! + "." + widget;
  ...
  withXcodeChanges(config, {
    ...
    cwd:
      ...
      path.relative(
        (config._internal as any).projectRoot,
        path.resolve(props.directory as any)
      ),
