...
function getMainMarketingVersion(project: XcodeProject) {
  ...
  if (!version || version === "$(MARKETING_VERSION)") {
    ...
    return (config as any).props.buildSettings.MARKETING_VERSION;
  }
...
function createWatchAppConfigurationList(
  ...
  const common: BuildSettings = {
    ...
    INFOPLIST_KEY_WKCompanionAppBundleIdentifier: (mainAppTarget as any).props
...
function addFrameworksToDisplayFolder(
  ...
  frameworks.forEach((file) => {
    if (
      !(mainFrameworksGroup as any).props.children.find(
        (child: any) => child.getDisplayName() === file.getDisplayName()
      )
    ) {
      (mainFrameworksGroup as any).props.children.push(file);
    }
  });
}
...
async function applyXcodeChanges(
  ...
  const magicCwd = path.join(
    (config._internal as any).projectRoot!,
    "ios",
    props.cwd
  );
    ...
    fileRef: PBXFileReference.create(project, {
      explicitFileType:
        props.type === "appintent"
          ? ("wrapper.extensionkit-extension" as any)
  ...
  const widgetTarget = project.rootObject.createNativeTarget({
    ...
    productType: productTypeForType(props.type) as any,
  });
  ...
  // TODO: Idempotent
  (mainSourcesBuildPhase as any).props.files.push(...intentBuildFiles[1]);
  ...
  const protectedGroup = ensureProtectedGroup(project).createGroup({
    ...
    children: [
      ...swiftFiles
        .map((buildFile: any) => buildFile.props.fileRef)
        .sort((a: any, b: any) =>
