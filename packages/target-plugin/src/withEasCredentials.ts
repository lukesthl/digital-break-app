...
export const withDefaultAppGroup: ConfigPlugin = (config) => {
  withEntitlementsPlist(config, (config) => {
    ...
    config.modResults["com.apple.security.application-groups"] = unique([
      ...
      "group." + (config.ios as any).bundleIdentifier!,
    ]);
...
export const withEASTargets: ConfigPlugin<{
  ...
  const existing = (
    config.extra as any
  ).eas.build.experimental.ios.appExtensions.findIndex(
  ...
  if (existing > -1) {
    ...
    (config.extra as any).eas.build.experimental.ios.appExtensions.push({
      bundleIdentifier,
      targetName,
    });
  }
